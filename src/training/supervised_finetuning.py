...
def main():
    ...
    with PartialState().local_main_process_first():
        if args.debug:
            ...
            train_dataset = train_dataset.map(format_fn, remove_columns=train_dataset.column_names,batched=True,batch_size=128 )
            eval_dataset = load_dataset(args.dataset, split="test[:100]")
            eval_dataset = eval_dataset.map(format_fn, remove_columns=eval_dataset.column_names ,batched=True,batch_size=128)
        else:
